...
Node.js bindings for libalpm (Arch Linux Package Manager) via napi-rs + the `alpm` Rust crate. The npm package name is `libalpm`. The end goal is to build an AUR helper.
...
Uses the standard napi-rs platform packages approach. Each target gets its own npm package (`libalpm-linux-x64-gnu`, etc.) published as `optionalDependencies`. The generated `index.js` detects the platform at runtime and loads the correct `.node` binary.

Targets (glibc only — libalpm is Linux-only):
- `x86_64-unknown-linux-gnu`
- `aarch64-unknown-linux-gnu`

The `npm/` directory contains the per-platform package definitions (generated by `napi create-npm-dirs`). CI builds the native binary for each target, uploads them as artifacts, then publishes platform packages before the main package.

Release flow: `gh release create v0.x.x --generate-notes` → creates GitHub Release + tag → triggers CD → builds → publishes to npm. Version is derived from the tag at publish time (not from package.json).

CI build strategy:
- x86_64: builds and tests directly in `archlinux:latest` container
- aarch64: builds via QEMU emulation using `uraimo/run-on-arch-action` with `archarm_latest` (no tests — no local pacman DB in emulated environment)
